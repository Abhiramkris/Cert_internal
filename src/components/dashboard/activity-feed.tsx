  ...
  return (
    <Card className="rounded-[2.5rem] border-none bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col h-full relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-8">
        <h3 className="text-xl font-semibold tracking-tight text-zinc-900">Activity & notifications</h3>
        <button 
          onClick={() => fetchNotifications()}
          className="flex items-center gap-2 text-[12px] font-semibold text-zinc-600 hover:text-zinc-600 transition-colors"
        >
      ...
      <div className="flex bg-zinc-50/50 p-1 rounded-[1.25rem] border border-zinc-100 mb-8 inline-flex w-full overflow-x-auto no-scrollbar">
        {tabs.map((tab) => (
          <button
            ...
            className={cn(
              "flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-[1rem] text-[13px] font-semibold transition-all whitespace-nowrap",
              activeTab === tab.id
                ? "bg-white text-zinc-900 shadow-sm border border-zinc-200/50"
                : "text-zinc-700 hover:text-zinc-700 hover:bg-zinc-100/50"
            )}
          >
            {tab.id}
            <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-semibold", tab.bg, tab.color)}>
              {tab.count}
            </span>
          </button>
        ))}
      ...
      <div className="flex-1 flex flex-col min-h-[300px]">
        <h4 className="text-[13px] font-semibold text-zinc-900 mb-6 px-1">{activeTab} Feed</h4>
        
        {loading && notifications.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-zinc-600 text-sm font-semibold">
            Loading...
          </div>
                ...
                <div key={item.id} className="relative flex items-start gap-4 group">
                  ...
                  <div className="flex flex-col min-w-0 pt-1 w-full">
                    <p className="text-[13px] text-zinc-600 leading-snug break-words">
                      <span className="font-semibold text-zinc-900 mr-1">System</span>
                      {item.message}
                    </p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[11px] font-semibold text-zinc-700">Project:</span>
                      <a href={`/dashboard/projects/${item.project_id}`} className="flex items-center text-[12px] font-semibold text-blue-600 hover:text-blue-700 hover:underline truncate">
                        {item.type.replace('_', ' ')} Context
                      </a>
                    </div>
                    <div className="mt-2 text-[10px] font-semibold text-zinc-600 uppercase tracking-wider">
                      {timeString}
                    </div>
                  </div>
                </div>
