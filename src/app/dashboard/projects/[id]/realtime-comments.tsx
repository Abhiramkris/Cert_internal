      ...
      <CardHeader className="bg-white border-b border-zinc-100 py-6 px-8 flex flex-row items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-[0.2em]">Communication Hub</span>
            <CardTitle className="text-lg font-semibold text-zinc-900 uppercase tracking-[0.1em] italic">Project Activity</CardTitle>
          </div>
        </div>
        <div className="flex bg-zinc-50 p-1 border border-zinc-100 rounded-none">
          <button 
            ...
            className={cn(
              "px-6 py-2.5 rounded-none text-[9px] font-semibold uppercase tracking-[0.2em] transition-all flex items-center gap-2",
              isInternal ? "bg-zinc-950 text-white shadow-md" : "text-zinc-600 hover:text-zinc-600"
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "px-6 py-2.5 rounded-none text-[9px] font-semibold uppercase tracking-[0.2em] transition-all flex items-center gap-2",
              !isInternal ? "bg-blue-600 text-white shadow-md" : "text-zinc-600 hover:text-zinc-600"
            )}
          >
        ...
        {comments?.length > 0 ? (
          comments.map((comment, i) => (
            <div key={i} className={cn(
               ...
               <div className={cn(
                 "w-12 h-12 border flex items-center justify-center text-[10px] font-semibold uppercase tracking-widest shrink-0 rounded-none transition-all group-hover:bg-zinc-950 group-hover:text-white",
                 comment.user_id ? "bg-white text-zinc-600 border-zinc-100" : "bg-blue-50 text-blue-600 border-blue-100"
               )}>
               ...
               <div className={cn("space-y-3 max-w-[75%]", comment.user_id === userId ? "items-end flex flex-col" : "")}>
                 <div className={cn("flex items-center gap-4", comment.user_id === userId ? "flex-row-reverse" : "")}>
                   <span className="text-[9px] font-semibold text-zinc-600 uppercase tracking-[0.3em]">
                     {comment.user_id === userId ? 'Authorizing Agent' : comment.user_id ? 'Team Proxy' : 'Client Terminal'}
                   </span>
                   <span className="text-[8px] text-zinc-500 font-semibold uppercase tracking-widest tabular-nums">{new Date(comment.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                   {comment.is_internal ? (
                     <Badge variant="outline" className="text-[8px] h-5 border-zinc-100 text-zinc-500 font-semibold bg-zinc-50 rounded-none px-3 tracking-[0.2em]">INTERNAL</Badge>
                   ) : (
                     <Badge variant="outline" className="text-[8px] h-5 border-blue-100 text-blue-400 font-semibold bg-blue-50 rounded-none px-3 tracking-[0.2em]">EXTERNAL</Badge>
                   )}
                 </div>
                 <div className={cn(
                   "p-6 border transition-all text-[11px] font-semibold tracking-widest uppercase leading-relaxed rounded-none shadow-sm whitespace-pre-wrap",
                   ...
                   {(comment.content || '').split(/((?:@|#)\w+)/g).map((chunk, chunkIdx) => {
                     if (chunk.startsWith('@')) {
                       return <span key={chunkIdx} className="text-blue-400 font-semibold">{chunk}</span>
                     }
                     if (chunk.startsWith('#')) {
                       return <span key={chunkIdx} className="text-emerald-400 font-semibold">{chunk}</span>
                     }
                     return chunk
                   })}
                 </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-12 text-zinc-500">
            ...
            <p className="text-sm font-semibold uppercase tracking-widest opacity-40">No activity logged</p>
            <p className="text-[10px] font-semibold mt-2 opacity-30 uppercase tracking-tight">Post a message to initiate collaboration</p>
          </div>
        )}
      </CardContent>
      ...
      <div className="p-8 bg-white border-t border-zinc-100 shrink-0 shadow-[0_-4px_24px_-12px_rgba(0,0,0,0.05)] relative">
        ...
        {mentionQuery !== null && (
          <div className="absolute bottom-full mb-2 left-8 w-64 bg-white border border-zinc-200 shadow-xl rounded-xl overflow-hidden z-50">
            {filteredItems.length > 0 ? (
              <div className="flex flex-col">
                <div className="px-3 py-2 bg-zinc-50 border-b border-zinc-100 text-[9px] font-semibold uppercase tracking-widest text-zinc-600">
              ...
              <div className="px-4 py-3 text-xs text-zinc-700 font-medium">No results found.</div>
            )}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="relative">
          <Textarea 
            ...
            className="min-h-[120px] bg-[#fafafa] border-zinc-200 rounded-none pr-16 pl-8 py-6 focus:ring-0 focus:border-zinc-950 text-[11px] font-semibold uppercase tracking-widest transition-all resize-none placeholder:text-zinc-200 text-zinc-900 leading-relaxed shadow-none"
          ...
          <PendingButton 
            ...
            type="submit" 
            size="sm" 
            className="absolute bottom-6 right-6 rounded-none h-11 px-6 bg-zinc-950 border border-zinc-800 font-semibold text-[10px] uppercase tracking-[0.2em] text-white shadow-xl hover:bg-black active:scale-95 transition-all"
        ...
        <div className="flex items-center justify-between mt-6 px-1">
          <div className="flex items-center gap-6 overflow-x-auto no-scrollbar">
            <p className="text-[8px] text-zinc-500 font-semibold uppercase tracking-[0.2em] whitespace-nowrap">
